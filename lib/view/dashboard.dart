                      ...
                      SizedBox(
                        ...
                        child: schoolStats.when(
                          ...
                          data: (stats) {
                            ...
                            int totalLiveStudents = schoolList.fold(0, (sum, item) {
                              if (item["Status"] == "LIVE" && item["isActive"] == "1"  ) {
                                return sum + (int.tryParse(item["Students"]?.toString() ?? "0") ?? 0);
                              }
                              return sum;
                            });
                            int totalLiveStaff = schoolList.fold(0, (sum, item) {
                              if (item["Status"] == "LIVE" && item["isActive"] == "1") {
                                return sum + (int.tryParse(item["Staff"]?.toString() ?? "0") ?? 0);
                              }
                              return sum;
                            });

                            int totalPOCStudents = schoolList.fold(0, (sum, item) {
                              if (item["Status"] == "POC") {
                                return sum + (int.tryParse(item["Students"]?.toString() ?? "0") ?? 0);
                              }
                              return sum;
                            ...
                            int totalStoppedStudents = schoolList.fold(0, (sum, item) {
                              if (item["Status"] == "STOPPED") {
                                return sum + (int.tryParse(item["Students"]?.toString() ?? "0") ?? 0);
                              }
                              return sum;
                            });
                            int totalStoppedStaff = schoolList.fold(0, (sum, item) {
                              if (item["Status"] == "STOPPED") {
                                return sum + (int.tryParse(item["Staff"]?.toString() ?? "0") ?? 0);
                              }
                              return sum;
                          ...
                          return DashboardTile(
                            ...
                            onTap: () {
                              switch (item.id) {
                                case 1:
                                  Navigator.pushNamed(
                                      context, RoutesName.createdemo);
                                  break;

                                case 2:
                                  Navigator.pushNamed(
                                      context, RoutesName.demolistview);
                                  break;

                                case 3:
                                  Navigator.pushNamed(
                                      context, RoutesName.schoollistview);
                                  break;

                                case 4:
                                  Navigator.pushNamed(
                                      context, RoutesName.circularlist);
                                  break;
                              }
                            },
                          );
